...
// corrupted pages → "fatal error: invalid runtime symbol table". A
// content-addressed snapshot is never overwritten (a rebuilt host
// binary hashes differently → a brand-new path), so an in-flight
// worker keeps executing its own stable copy; the next task hashes the
// new binary, the generated VM config changes, and the existing
// drift-recreate rebuilds the VM. It also narrows the lima mount to a
// dir containing only enso (no more whole-~/go/bin exposure).
package exestage
...
// $XDG_STATE_HOME/enso/exe/<sha256[:16]>/enso, copying it once if
// absent. Idempotent and content-addressed: identical binary content
// always maps to the same path and is copied at most once; a different
// (rebuilt) binary gets a brand-new path and never overwrites an
// existing snapshot a running guest may still be executing.
func Stage(exe string) (string, error) {
	...
	dir := filepath.Join(sd, dirName, sum)
