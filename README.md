...
- `bash(git *)` — first-word match for single tokens, full-glob for multi-word (`bash(git push *)` properly scopes to `git push`). **Allow rules also gate on shell metacharacters**: any of `;` `&` `|` `<` `>` `$` `` ` `` `(` `)` `\` newline present in the command must also appear in the pattern, so `bash(git *)` will NOT auto-allow `git status; rm -rf ~`. Opt in explicitly with patterns like `bash(git * | *)` if you need pipes. Ask/deny rules are not gated this way. *This is not a substitute for the bash sandbox* — for real isolation against a hostile model, set `bash.sandbox = "auto"` (a deny rule like `bash(rm -rf *)` can still be bypassed with a prepended command, since closing that needs a real shell parser).
